...
def make_cli_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--save-dir",
        help="directory to save the test wildfire dataset.",
        type=Path,
        default=Path("./data/processed/wildfire_test/"),
    )
    parser.add_argument(
        "--dir-previous-test-dataset",
        help="directory containing the previous test dataset.",
        type=Path,
...
def validate_parsed_args(args: dict) -> bool:
    ...
    """
    if not args["dir_previous_test_dataset"].exists():
        logging.error(
            f"invalid --dir-previous-test-dataset, dir {args['dir_previous_test_dataset']} does not exist"
        )
        return False
...
def add_percentage_background_images(
    ...
    test_sampled_false_positives = rng.sample(
        data_split_background.test,
        min(
            len(data_split_background.test),
            int(len(data_split_smoke.test) * percentage_background),
        ),
    )
    return DataSplit(
        train=[],
        val=[],
        test=data_split_smoke.test.copy() + test_sampled_false_positives,
    )
...
def persist_data_split(
    ...
    Returns:
        None
    """
    for split, split_xs in [("test", data_split.test)]:
...
if __name__ == "__main__":
    ...
    if not validate_parsed_args(args):
        ...
        filepaths_previous_test_dataset_images = list_ultralytics_images(
            dir_previous_test_dataset
        )
        filepaths_false_positive_images = list_ultralytics_images(
            dir_data_split_false_positives
        )
        logger.info(
            f"found {len(filepaths_previous_test_dataset_images)} images in {dir_previous_test_dataset}"
        )
        logger.info(
            f"found {len(filepaths_false_positive_images)} images in {dir_data_split_false_positives}"
        )
        data_split_previous_test_dataset = load_data_split_from_ultralytics(
            dir_previous_test_dataset
        )
        ...
        data_split = add_percentage_background_images(
            data_split_smoke=data_split_previous_test_dataset,
